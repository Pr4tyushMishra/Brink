...
export const renderCenteredText = (ctx: CanvasRenderingContext2D, e: CanvasEntity, textColor = '#1f2937') => {
    if (!e.props.text) return;
    ...
    // For shapes like triangles or diamonds, we restrict the max width even further than just padding
    const typeCompression = (e.type === 'TRIANGLE' || e.type === 'DIAMOND' || e.type === 'ELLIPSE') ? 0.6 : 1.0;
    const maxWidth = (e.props.width * typeCompression) - padding * 2;
    ...
    let startY = e.transform.y + (e.props.height / 2) - (totalHeight / 2) + (lineHeight / 2);

    for (const line of wrappedLines) {
        ctx.fillText(line, e.transform.x + (e.props.width / 2), startY);
        startY += lineHeight;
    }
